    ...
    try {
      ...
      if (register) {
        const res = await fetch('/api/v1/auth/register', {
          ...
          body: JSON.stringify({ username: account, password, email: account.includes('@') ? account : '' }),
        });
        if (!res.ok) {
