...
import { ArrowRight, Camera, CheckCircle2, Eye, EyeOff, LockKeyhole, Mail, ShieldCheck, UserRound, Waves } from 'lucide-react';
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  // 忘记密码：step1 验证账号（用户名+邮箱双要素，手机号选填不参与）→ step2 设置新密码（两次输入）
  ...
  const doFaceLogin = async () => {
    setError('');
    // 人脸登录双因子：先输入账号（用户名/手机号/邮箱）锁定范围，再刷脸验证
    // （一张人脸可录入多个账号，不输账号会登进"最近录入的同脸账号"）
    const account = username.trim();
    if (!account) {
      setError('请先输入用户名/手机号/邮箱，再进行人脸识别登录');
      return;
    }
    setFaceBusy(true);
    try {
      const file = faceCam.capture();
      const result = await api.faceLogin(file, account);
  ...
  /** 忘记密码提交：双要素（用户名+邮箱）+ 两次新密码一并交给后端校验与重置 */
  const submitReset = async (event: FormEvent) => {
    ...
    try {
      const res = await fetch('/api/v1/auth/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: fAccount.trim(), email: fEmail.trim(), new_password: fPwd1, confirm_password: fPwd2 }),
      });
      if (!res.ok) {
        const payload = await res.json().catch(() => null);
