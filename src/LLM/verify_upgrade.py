...
def run_offline_regression() -> None:
    ...
    _require("海里塑料袋多久会真正消失？", "没有一个可靠、统一", "碎裂成微塑料")
    _require("珊瑚边发现遗失渔网，现场处置的第一步和注意事项是什么？", "记录位置", "不要直接拖拽")
    _require("请给一份海滩垃圾处置的优先级框架。", "先评估、再分区、后清理、再复测", "优先处理")
    _require("我想知道今天股票会怎么走。", "不在我的知识范围内")
