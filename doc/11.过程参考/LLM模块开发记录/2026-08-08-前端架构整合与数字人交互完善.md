...
**根因**: 同上个 session 的已知问题（见 `2026-08-07-LLM模块工作日志.md` 问题4），但修复未带入迁移后的 `knowledge_base.py`。`.md` 文件加载器仍为 `UnstructuredMarkdownLoader`，其依赖 `markdown` 包未安装于 xa_code 环境。
...
| （共 5 条标准查询，类 `2026-08-07-LLM模块工作日志.md` 3c 节结果） | — |
...
4. **`config.py` 统一**: 当前 `src/backend/config.py` 仍为旧版（无 Settings 类），`knowledge_base.py` 已绕开但 `2026-08-07-LLM模块工作日志.md` 规划的 Settings 统一配置中心未落地。待后续 session 评估是否仍需引入 `python-dotenv` + Settings 数据类
